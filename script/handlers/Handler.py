...
class Handler():
    ...
    def on_connection(self, client, server):
        ...
        """
        self.control_surface.log_message("New client connected and was given id %d" %
                                         client['id'])
        self.client_codes.new(client['id'])

        server.send_message(client, OutgoingMessage(
            Status.SUCCESS,
            Method.AUTH,
            "/socket",
            "info",
        ...
        if message.method == Method.AUTH:
            ...
            if message.address == "/code" and message.prop == "check":
                if self.client_codes.validate(client["id"], message.code):
                    server.send_message(client, OutgoingMessage(
                        Status.SUCCESS, message.method, message.address, message.prop, message.code).data)
                else:
                    server.send_message(client, OutgoingMessage(
                        Status.FAILURE, message.method, message.address, message.prop, "Incorrect code").data)
                return
            ...
            if message.address == "/pref":
                ...
                value = getattr(self.locate(message.address),
                                message.prop)

            if value is not None:
                server.send_message(client, OutgoingMessage(
                    Status.SUCCESS, message.method, message.address, message.prop, value).data)
                return

        elif message.method == Method.LISTEN:
            if len([listener for listener in self.listeners if listener.client['id']
                    == client['id'] and listener.prop == message.prop]) == 0:

                def callback():
                    result = getattr(self.locate(message.address),
                                     message.prop)

                    if result is not None:
                        try:
                            server.send_message(client, (OutgoingMessage(
                                Status.SUCCESS, message.method, message.address, message.prop, result)).data)
                ...
                try:
                    ...
                    server.send_message(client, (OutgoingMessage(
                        Status.FAILURE, message.method, message.address, message.prop, str(error))).data)
                    return
            else:
                server.send_message(client, (OutgoingMessage(
                    Status.FAILURE, message.method, message.address, message.prop, "listener already exists")).data)
                return

        elif message.method == Method.UNLISTEN:
            listener = first([listener for listener in self.listeners if listener.client['id']
                              == client['id'] and listener.prop == message.prop])
            ...
            if listener is not None:
                ...
                self.listeners.remove(listener)
            server.send_message(client, (OutgoingMessage(
                Status.SUCCESS, message.method, message.address, message.prop)).data)
            return

        if self.control_surface.preferences.get("requireCode") is True and self.client_codes.validate(client["id"], message.code) is False:
            server.send_message(client, OutgoingMessage(
                Status.FAILURE, message.method, message.address, message.prop, "Not authorized").data)
            return
            ...
            if message.value is not None:
                try:
                    if message.address == "/pref":
                        ...
                        setattr(self.locate(message.address),
                                message.prop, message.value)

                    server.send_message(client, OutgoingMessage(
                        Status.SUCCESS, message.method, message.address, message.prop, message.value).data)
                    return
                ...
                server.send_message(client, OutgoingMessage(
                    Status.FAILURE, message.method, message.address, message.prop, "No value was given").data)
                return
