...
class WebSocketHandler(StreamRequestHandler):
    ...
    def __init__(self, socket, addr, server):
        ...
        if server.key and server.cert:
            try:
                ...
                logger.warning("SSL not available (are the paths {} and {} correct for the key and cert?)".format(
    ...
    def send_text(self, message, opcode=OPCODE_TEXT):
        ...
        if isinstance(message, bytes):
            ...
            logger.warning(
                'Can\'t send message, message has to be a string or bytes. Got %s' % type(message))
            return False
